...
const deviceCodes = new Map<string, DeviceCodeData>();
const userCodes = new Map<string, string>(); // Maps user codes to device codes
...
export function authorizeDeviceCode(userCode: string, userId: string): boolean {
    ...
    deviceCodes.set(deviceCode, updatedData);
    return true;
}
...
export function cleanupExpiredCodes(): void {
    ...
    for (const [code, data] of deviceCodes.entries()) {
        if (data.expiresAt < now && data.status !== DeviceCodeStatus.EXPIRED) {
            // Mark as expired
