...
/**
 * Register OpenID Connect Example Client
 *
 * This script registers the OpenID Connect example client with the OAuth2 server.
 * It creates a persistent client with the necessary configuration for the example.
 *
 * Usage:
 *   npm run register-openid-client
 */
...
async function main() {
    console.log('Registering OpenID Connect Example Client\n');

    // Initialize storage
    storage.initializeTestClient();

    // Check if client already exists
    const existingClient = storage.getClient('openid-example-client');

    if (existingClient) {
        console.log('OpenID Connect Example Client already exists:');
